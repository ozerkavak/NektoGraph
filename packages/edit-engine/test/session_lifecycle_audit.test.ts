...
import { ICommitStrategy } from '../../session/src/types';
import { IDFactory } from '@triplestore/core';

describe('Sentinel: Session Lifecycle Consistency Audit', () => {
    let manager: SessionManager;
    let mockStrategy: ICommitStrategy;
    ...
    it('AUDIT: closeSession MUST reject non-string IDs to prevent PointerEvent leaks', () => {
        ...
        const fakeEvent = { type: 'click', target: {} } as any;
    ...
    it('AUDIT: cancelSession logic wrap (Simulation of AppState bug)', () => {
        ...
        let currentSession: any = manager.createSession('test-user');
        const sessionId = currentSession.id;

        const cancelSession = (id: any) => {
        ...
        cancelSession({ type: 'click' } as any);
