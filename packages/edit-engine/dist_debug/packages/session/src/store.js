...
export class CompositeStore {
    ...
    *match(subject, predicate, object, graph) {
        ...
        for (const q of this.main.match(subject, predicate, object, graph)) {
            if (this.draft) {
                const key = `${q[0]}_${q[1]}_${q[2]}_${q[3]}`;
        ...
        // 2. Iterate Draft
        if (this.draft) {
            // Note: This yields everything in draft additions.
            // If draft additions duplicate main store items (that weren't deleted), we assume Set semantics upstream handles it,
            // or we accept duplicates in Composite View for now (Performance tradeoff).
            // Deduplication requires keeping a Set of yielded hashes, which costs memory.
            yield* this.draft.match(subject, predicate, object, graph);
        }
    }
    // Read-Only Stubs
