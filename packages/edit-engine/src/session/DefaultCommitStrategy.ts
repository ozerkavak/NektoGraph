import { IQuadStore, IDFactory, Quad, DEFAULT_GRAPH } from '@triplestore/core';
...
export class DefaultCommitStrategy implements ICommitStrategy {
    constructor(
    ...
    async execute(draft: DraftStore): Promise<void> {
        ...
        for (const key of draft.deletions) {
            ...
            diffQuads.push(
                { subject: bnode, predicate: this.factory.namedNode('http://www.w3.org/1999/02/22-rdf-syntax-ns#type'), object: this.factory.namedNode(TYPE_DELETION), graph: g },
                { subject: bnode, predicate: this.factory.namedNode(PROP_SUBJECT), object: s, graph: g },
                { subject: bnode, predicate: this.factory.namedNode(PROP_PREDICATE), object: p, graph: g },
                { subject: bnode, predicate: this.factory.namedNode(PROP_OBJECT), object: o, graph: g },
                { subject: bnode, predicate: this.factory.namedNode(PROP_TIMESTAMP), object: this.factory.literal(timestamp), graph: g }
            );
        }
        ...
        for (const raw of draft.additions.match(null, null, null, null)) {
            ...
            const finalQuad: Quad = { ...q, graph: targetGraph };
    ...
    private isDefaultGraph(g: any): boolean {
        return !g || g === DEFAULT_GRAPH;
    }

    private findAffinityGraph(subject: any): any {
