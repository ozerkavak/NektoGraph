...
/**
 * GraphOption: Data structure for a selectable graph entry.
 */
export interface GraphOption {
    uri: string;       // Value to return (Internal/Target ID)
    label: string;     // Primary Display Name (Short/Linked Name)
    fullUri: string;   // Secondary identifier (Logical/Full URI)
    type: string;      // Category (data, ontology, etc.)
    sourceType: string; // Physical origin (local, system, remote)
    location: string;   // Source location or filename
    
    mainCount?: number;  // Metric: Triple count in store
    draftCount?: number; // Metric: Count in current session
    isDefault?: boolean;
    defaultLabel?: string; // Opt text for the default choice banner
}
...
export class GraphSelector {
    ...
    private static USER_GRAPH_URI = 'http://example.org/graphs/user';

    public static async request(params: SelectorParams): Promise<string | null> {
        ...
        return new Promise((resolve) => {
            ...
            const recommended = options.filter(o => 
                o.isDefault || (o.type === 'data' && (o.sourceType === 'local' || o.sourceType === 'remote')) || (o.type === 'ontology' && o.isDefault)
            ).filter(o => (o.uri !== this.USER_GRAPH_URI && o.label !== 'user') || o.isDefault);
            ...
            const userGraphEntry = options.find(o => 
                (o.uri === this.USER_GRAPH_URI || o.label === 'user') && 
                !recommended.find(r => r.uri === o.uri)
            );

            // Group 3: Others (Ontology, system, etc)
            const others = options.filter(o => 
                !recommended.find(r => r.uri === o.uri) && 
            ...
            const renderOption = (opt: GraphOption, container: HTMLElement) => {
                ...
                btn.onclick = (e: MouseEvent) => {
                    ...
                    if (target.closest('.gs-copy-btn')) {
                        ...
                        const copyBtn = target.closest('.gs-copy-btn') as HTMLElement;
                        copyBtn.innerText = '✅';
            ...
            `;
            const newGraphInput = newGraphSection.querySelector('.gs-main-input') as HTMLInputElement;
            newGraphInput.value = selectedUri;
