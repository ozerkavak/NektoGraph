...
export class WebGLContainer {
    ...
    private options: WebGLContainerOptions;

    // State
    private isRunning: boolean = false;
    // Camera State
    private cameraTheta: number = 0; // Horizontal orbit
    private cameraPhi: number = Math.PI / 3;
    private cameraRadius: number = 25;
    private target: [number, number, number] = [0, 0, 0];
    private startTime: number = Date.now();
    private showEmptyProps: boolean = false;
    ...
    private capturedNodeId: string | null = null;
    ...
    private instanceCount = { cube: 0, pyramid: 0, sphere: 0, inverted_pyramid: 0 };
    ...
    private moveNode(id: string, dx: number, dy: number) {
    ...
    private getRayFromNDC(ndcX: number, ndcY: number): Ray {
        const invVP = this.projMatrix.multiply(this.viewMatrix).invert();
        const pNear = invVP.multiplyVec4([ndcX, ndcY, -1, 1]);
        const pFar = invVP.multiplyVec4([ndcX, ndcY, 1, 1]);

        const near3: [number, number, number] = [pNear[0] / pNear[3], pNear[1] / pNear[3], pNear[2] / pNear[3]];
        const far3: [number, number, number] = [pFar[0] / pFar[3], pFar[1] / pFar[3], pFar[2] / pFar[3]];

        const dir: [number, number, number] = [far3[0] - near3[0], far3[1] - near3[1], far3[2] - near3[2]];
    ...
    private createMeshVAO(vertices: Float32Array): WebGLVertexArrayObject | null {
        const gl = this.gl!;
    ...
    private setupInstanceAttributes(instanceOffset: number = 0) {
        const gl = this.gl!;
        gl.bindBuffer(gl.ARRAY_BUFFER, this.instanceBuffer);
        const stride = 7 * 4;
    ...
    public updateGraph(data: ThreeDGraphData) {
        ...
        this.nodes = data.nodes.map(newNode => {
            ...
            if (existing && existing.position) {
                return { ...newNode, position: [...existing.position] as [number, number, number] };
            }
            return newNode;
        });
    ...
    private syncGPUBuffer() {
        if (!this.gl || !this.instanceBuffer || !this.lineBuffer) return;
        const gl = this.gl;
    ...
    private render() {
        ...
        if (this.uProjection) gl.uniformMatrix4fv(this.uProjection, false, projection.data);
        if (this.uView) gl.uniformMatrix4fv(this.uView, false, view.data);
        if (this.uTime) gl.uniform1f(this.uTime, (Date.now() - this.startTime) / 1000.0);
    ...
    private renderBackground(projection: Mat4, view: Mat4) {
        if (!this.bgProgram || !this.bgVAO) return;
        const gl = this.gl!;
        ...
        if (locProj) gl.uniformMatrix4fv(locProj, false, projection.data);
        if (locView) gl.uniformMatrix4fv(locView, false, view.data);
    ...
    private renderEdges(projection: Mat4, view: Mat4) {
        if (!this.lineProgram || !this.lineVAO || this.lineCount === 0) return;
        const gl = this.gl!;
        ...
        if (locProj) gl.uniformMatrix4fv(locProj, false, projection.data);
        if (locView) gl.uniformMatrix4fv(locView, false, view.data);
        if (locOpacity) gl.uniform1f(locOpacity, 0.6);

        const locTime = gl.getUniformLocation(this.lineProgram, 'u_time');
        if (locTime) gl.uniform1f(locTime, (Date.now() - this.startTime) / 1000.0);
    ...
    public getNodeById(id: string): Node3DData | undefined {
        return this.nodes.find(n => n.id === id);
    }

    public getNodes(): Node3DData[] {
        return this.nodes;
    }
}
