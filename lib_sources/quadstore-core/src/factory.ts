import { IDataFactory, NodeID, NodeType, Token, TripleToken, MASK_TYPE, SHIFT_TYPE } from './types';
...
export class IDFactory implements IDataFactory {
    private uriPool = new StringPool();
    private bnodePool = new StringPool();


    private literalMap = new Map<string, number>();
    private literalArray: Token[] = [];

    private tripleMap = new Map<string, number>();
    private tripleArray: TripleToken[] = [];

    // Star Inverted Indices (Constituent NodeID -> List of Triple NodeIDs)
    private starIdxS = new StarInvertedIndex();
    private starIdxP = new StarInvertedIndex();
    private starIdxO = new StarInvertedIndex();
    ...
    triple(subject: NodeID, predicate: NodeID, object: NodeID): NodeID {
        ...
        if (id === undefined) {
            ...
            const sStr = (this.decode(subject) as any).value || subject.toString();
            const pStr = (this.decode(predicate) as any).value || predicate.toString();
            const oStr = (this.decode(object) as any).value || object.toString();
    ...
    findQuotedTriples(constituent: NodeID, role?: 'S' | 'P' | 'O'): readonly NodeID[] {
        if (role === 'S') return this.starIdxS.get(constituent) || [];
        if (role === 'P') return this.starIdxP.get(constituent) || [];
        if (role === 'O') return this.starIdxO.get(constituent) || [];
